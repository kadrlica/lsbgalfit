...
ROW_STAR = """
    <tr>
      <th>{idx}</th>
      <td>
        <h4 style="color:red;">COADD_OBJECT_ID: <a href="http://legacysurvey.org/viewer/?ra={RA}&dec={DEC}&layer=ls-dr10&zoom=15">{COADD_OBJECT_ID}</a></h4>
        <h4 style="color:red;">RA,DEC: {RA:.4f}, {DEC:.4f}</h4>
        <h4 style="color:red;">MAG_AUTO (g,r,i): {MAG_AUTO_G:.2f}, {MAG_AUTO_R:.2f}, {MAG_AUTO_I:.2f}</h4>
        <h4 style="color:red;">MAG_GALFIT (g,r,i): {MAG_G:.2f}, {MAG_R:.2f}, {MAG_I:.2f}</h4>
        <h4 style="color:red;">COLOR_AUTO (g-r,g-i): {COLOR_AUTO_GR:.2f}, {COLOR_AUTO_GI:.2f}</h4>
        <h4 style="color:red;">COLOR_GALFIT (g-r,g-i): {COLOR_GR:.2f}, {COLOR_GI:.2f}</h4>
        <h4 style="color:red;">FLUX_RADIUS (g,r,i): {FLUX_RADIUS_G:.2f}, {FLUX_RADIUS_R:.2f}, {FLUX_RADIUS_I:.2f}</h4>
        <h4 style="color:red;">RE_G,RE_R,RE_I: {RE_G:.2f}, {RE_R:.2f}, {RE_I:.2f}</h4>
        <h4 style="color:red;">N_GALFIT: {N:.2f}</h4>
        <h4 style="color:red;">CHI2NU: {CHI2NU:.2f}</h4>
        <h4 style="color:red;">CHI2NU (g,r,i): {CHI2NU_G:.2f},{CHI2NU_R:.2f},{CHI2NU_I:.2f}</h4>
      </td>
      <td> <a id="{COADD_OBJECT_ID}"></a><a href="{results}"><img src="{results}" alt="LSB Galaxy Fit" width="600"></a></td>
      <td> <a id="{COADD_OBJECT_ID}"></a><a href="{segmap}"><img src="{segmap}" alt="LSB Galaxy Segmap" width="500"></a></td>
    </tr>  
...
if __name__ == "__main__":
    ...
    stars = pyfits.open(config['basedir']+'/'+config['starfile'])[1].data['COADD_OBJECT_ID']
